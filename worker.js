...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname.startsWith("/gh-api/")) {
      ...
      const tokenExiste = !!env.GITHUB_API_TOKEN;
      const tokenFinal = env.GITHUB_API_TOKEN ? env.GITHUB_API_TOKEN.slice(-4) : "AUSENTE";
      ...
      const corpoBruto = await respostaGitHub.text();

      return new Response(corpoBruto, {
        status: respostaGitHub.status,
        headers: {
          "Content-Type": respostaGitHub.headers.get("Content-Type") || "application/json",
          "Cache-Control": "public, max-age=60",
          "X-Debug-Token-Existe": String(tokenExiste),
          "X-Debug-Token-Final4": tokenFinal,
          "X-Debug-URL-Chamada": urlGitHub,
        },
      });
    }
